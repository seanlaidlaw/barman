...
#'
#' @export
filter_and_normalise_scRNA__seurat <- function(counts_matrix, output_dir="./", manual_filter=FALSE, filter_only=FALSE) {
	...
	seurat_obj = CreateSeuratObject(counts = SingleCellExperiment::counts(fc_sce))
	seurat_obj = NormalizeData(object = seurat_obj)
	norm_matrix = as.matrix(GetAssayData(object = seurat_obj, slot = 'data'))

	# remove non-endogenous genes
	endogenous_genes = rownames(fc_sce[!(SingleCellExperiment::isSpike(fc_sce, "ERCC") | SingleCellExperiment::isSpike(fc_sce, "MT"))])
	endogenous_genes = endogenous_genes[endogenous_genes %in% rownames(norm_matrix)]
	norm_matrix = norm_matrix[endogenous_genes,]
